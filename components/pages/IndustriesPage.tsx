...
export const IndustriesPage: React.FC = () => (
  <div style={{ paddingTop: 'var(--nav-h)', paddingBottom: 80 }}>
    ...
    {/* Hero */}
    <section style={{ padding: 'clamp(48px,6vw,88px) 0 56px', textAlign: 'center', background: 'var(--bg0)' }}>
      <div className="wrap" style={{ maxWidth: 720 }}>
        ...
        <Reveal delay={0.08}>
          <h1 style={{ fontFamily: 'var(--fd)', fontSize: 'clamp(2.25rem,5vw,3.75rem)', fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1.08, color: 'var(--t1)', marginBottom: 24 }}>
            Built For<br />
            <span style={{ color: 'var(--blue)' }}>Your</span>{' '}Market.
          </h1>
        </Reveal>
        <Reveal delay={0.14}>
          <p style={{ fontSize: 'clamp(1rem,1.5vw,1.125rem)', color: 'var(--t3)', lineHeight: 1.7, marginBottom: 36 }}>
    ...
    {/* Stats strip */}
    <section style={{ background: 'var(--bg1)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '24px 0' }}>
      <div className="wrap">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: '16px 32px', textAlign: 'center' }}>
          {STATS.map(s => (
            <div key={s.value}>
              <p style={{ fontFamily: 'var(--fd)', fontSize: 'clamp(1.25rem,2.5vw,1.75rem)', fontWeight: 900, color: 'var(--blue)', letterSpacing: '-0.03em', marginBottom: 4 }}>{s.value}</p>
              <p style={{ fontSize: '0.8rem', color: 'var(--t4)', letterSpacing: '0.01em' }}>{s.label}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section style={{ padding: 'clamp(48px,6vw,80px) 0' }}>
      <div className="wrap">
        <Reveal>
          <p style={{ fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: 'var(--t4)', marginBottom: 8 }}>Choose Your Industry</p>
          <h2 style={{ fontFamily: 'var(--fd)', fontSize: 'clamp(1.5rem,3vw,2.25rem)', fontWeight: 900, letterSpacing: '-0.03em', color: 'var(--t1)', marginBottom: 12 }}>Every industry has a different problem.</h2>
          <p style={{ color: 'var(--t3)', fontSize: '0.9375rem', marginBottom: 48 }}>We've built the right solution for each. Click any industry to see exactly how eighty5.OS solves your specific gaps.</p>
        </Reveal>
          ...
          {INDUSTRIES.map((ind, i) => (
            <Reveal key={ind.route} delay={i * 0.06}>
              ...
              >
                ...
                <p style={{ fontSize: '0.9rem', color: 'var(--t3)', lineHeight: 1.65, marginBottom: 24, flex: 1 }}>{ind.desc}</p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
                  {ind.keyStats.map(stat => (
                    <div key={stat} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                      ...
                      <p style={{ fontSize: '0.8rem', color: 'var(--t4)', lineHeight: 1.5 }}>{stat}</p>
                    </div>
                  ))}
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--blue)', fontSize: '0.875rem', fontWeight: 700, fontFamily: 'var(--fd)' }}>
