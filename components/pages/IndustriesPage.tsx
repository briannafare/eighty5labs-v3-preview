...
import {
  IconBank, IconYardSign, IconWrench, IconPulse, IconScales, IconArrowRight,
} from '../ui/Icons';
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 16 }}>
          {INDUSTRIES.map((ind, i) => (
            <Reveal key={ind.route} delay={i * 0.06}>
              <motion.div
                whileHover={{ borderColor: '#1B4FFF' }}
                onClick={() => navigate(ind.route)}
                style={{ padding: '32px 32px 28px', background: '#F7F9FF', border: '1px solid #DDE5F2', borderRadius: 'var(--rdl)', cursor: 'pointer', transition: 'border-color 0.2s', height: '100%', display: 'flex', flexDirection: 'column' }}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
                  <ind.Icon size={22} color="#64748B" />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
                  {ind.keyStats.map(stat => (
                    <div key={stat} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                      <span aria-hidden="true" style={{ width: 4, height: 4, borderRadius: '50%', background: '#94A3B8', marginTop: 7, flexShrink: 0 }} />
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#1B4FFF', fontSize: '0.875rem', fontWeight: 700, fontFamily: 'var(--fd)' }}>
                  See the full {ind.label} playbook <IconArrowRight size={14} />
                </div>
              </motion.div>
            </Reveal>
          ))}
