...
const FAQItem: React.FC<{ q: string; a: string }> = ({ q, a }) => {
  ...
  return (
    <div style={{ borderBottom: '1px solid #DDE5F2' }}>
      <button onClick={() => setOpen(o => !o)}
        ...
        <span style={{ fontWeight: 600, color: '#0F172A', fontSize: '0.9375rem', lineHeight: 1.4 }}>{q}</span>
        <span style={{ color: '#1B4FFF', fontSize: '1.125rem', minWidth: 20, paddingTop: 1, transition: 'transform 0.2s', transform: open ? 'rotate(45deg)' : 'rotate(0)' }}>+</span>
      </button>
      <motion.div initial={false} animate={{ height: open ? 'auto' : 0, opacity: open ? 1 : 0 }} style={{ overflow: 'hidden' }}>
        <p style={{ fontSize: '0.9rem', color: '#64748B', lineHeight: 1.7, paddingBottom: 20 }}>{a}</p>
      </motion.div>
    </div>
  );
};
...
export const IndustryPage: React.FC<{ data: IndustryPageData }> = ({ data }) => {
  ...
  return (
    <div style={{ paddingTop: 'var(--nav-h)', background: '#FFFFFF', color: '#0F172A', paddingBottom: 80 }}>
        ...
        <div className="wrap">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))', gap: 48, alignItems: 'center' }}>
            <div>
              ...
              <Reveal delay={0.14}>
                <p style={{ fontSize: 'clamp(0.9375rem,1.25vw,1.0625rem)', color: '#64748B', lineHeight: 1.7, marginBottom: 28, maxWidth: 520 }}>{data.heroCopy}</p>
              </Reveal>
              <Reveal delay={0.18}>
      ...
      {/* Stats strip */}
      <section style={{ background: '#F7F9FF', borderTop: '1px solid #DDE5F2', borderBottom: '1px solid #DDE5F2', padding: '20px 0' }}>
        <div className="wrap">
            ...
            <div>
              ...
              {data.problemBody.map((para, i) => (
                <Reveal key={i} delay={i * 0.05}>
                  <p style={{ fontSize: '0.9375rem', color: '#64748B', lineHeight: 1.75, marginBottom: 16 }}>{para}</p>
                </Reveal>
              ))}
              <Reveal delay={0.15}>
                <div style={{ marginTop: 24, padding: '20px 24px', background: '#F7F9FF', border: `1px solid ${accent}30`, borderLeft: `3px solid ${accent}`, borderRadius: 10 }}>
                  <p style={{ fontSize: '0.875rem', color: '#334155', fontStyle: 'italic', lineHeight: 1.6 }}>"{data.quote}"</p>
                  <p style={{ fontSize: '0.75rem', color: '#94A3B8', marginTop: 10 }}>In Their Own Words</p>
                </div>
              </Reveal>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {data.systems.map((sys, i) => (
                  <Reveal key={sys.name} delay={i * 0.05}>
                    <div style={{ padding: '18px 20px', background: '#F7F9FF', border: '1px solid #DDE5F2', borderRadius: 12 }}>
                      <p style={{ fontFamily: 'var(--fd)', fontWeight: 800, fontSize: '0.875rem', color: accent, marginBottom: 6 }}>{sys.name}</p>
                      <p style={{ fontSize: '0.875rem', color: '#64748B', lineHeight: 1.6 }}>{sys.desc}</p>
                    </div>
                  </Reveal>
                ))}
              </div>
      ...
      {/* Bullets + bold close */}
      <section style={{ padding: 'clamp(40px,4vw,60px) 0', background: '#F7F9FF', borderTop: '1px solid #DDE5F2', borderBottom: '1px solid #DDE5F2' }}>
        <div className="wrap" style={{ maxWidth: 800 }}>
          ...
          <p style={{ fontSize: '1rem', color: '#64748B', fontStyle: 'italic', borderTop: '1px solid #DDE5F2', paddingTop: 20 }}>{data.boldClose}</p>
        </div>
      </section>
